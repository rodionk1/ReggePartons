...
int main(int argc, char *argv[])
{
 ...
 int nR =0;
  ...
  bufferL->fullevolve(1.0*rapstart);
  bufferR->fullevolve(1.0*rapstart);
  ...
  /* Uniting buffers and computing amplitude in a block */
  ...
  }; // end of the block where bufferC is seen
  if(iev<Nevol) { bufferL->fullevolve(rapstep); bufferR->fullevolve(rapstep);};
    };
       delete bufferL;
       delete bufferR;
