...
int main(int argc, char *argv[])
{
  ...
  while(if2.get(sSD)){
    ...
    for (int irap=0; irap<rap; irap++){
      ...
      amplsd[irap][ibb]=0; 
      //amplsd[irap][ibb]+=as;
      amplsd[irap][ibb]+=2*ampl[irap][ibb]-as;
    };
    //      std::cout << std::endl;
    ibb++;
    if2.getline(s1SD,1000);
  ...
  */
  ...
  double Deltaplus = -0.32;   // PDG -0.34 Volkovitsky -0.55 T-M -0.27 Poghosyan -0.3                                                                                      
  double alplus = 0.70*0.197*0.197; // Volkovitsky 0.70   T-M 0.70  Poghosyan 0.8                                                                                          
  double betaplus2=8.5/(4*pi); // PDG 6.01                                                                                                                                
  // betaplus2=0;                                                                                                                                                         
  double rplus = 3.0*0.197*0.197; // Volkovitsky 1.55 T-M 3.00 Poghosyan 0.918                                                                                            
  double Deltaminus = -0.54;   // PDG -0.55 Volkovitsky -0.57 T-M -0.54 Poghosyan -0.6                                                                                     
  double alminus = 1.0*0.197*0.197; // Volkovitsky 1.0 T-M 1.0  Poghosyan 0.9                                                                                              
  double betaminus2= 3.5/(4*pi); //PDG 3.28                                                                                                                               
  // betaminus2=0;                                                                                                                                                        
  double rminus = 9.5*0.197*0.197; // Volkovitsky 5.19 T-M 10.00 Poghosyan 0.945                                                                                            
  ...
  for (int irap =0; irap<rap; irap++){
    // uncomment for profile    std::cout << rapstart+irap*rapstep <<" " ;    
    ...
    for(int ii=0;ii<ibb; ii++){
     ...
     //      ampl[irap][ii] = amplP;
      ...
      ampl[irap][ii] = amplP +(regplus+regminus)-amplP*(regplus+regminus); // Im part of pp amplitude                                                                      
      amplppbar[irap][ii]=amplP +(regplus-regminus)-amplP*(regplus-regminus); // Im part of ppbar amplitude                                


     //       std::cout<<bb[ii]<<" " << amplsd[irap][ii] << " " << std::endl;
      ...
      /* repp -- real part of the pp secondary reggeon exchange amplitude  */
      /* reppbar -- real part of the ppbar secondary reggeon exchange amplitude */
      ampl2[irap][ii]=pow(ampl[irap][ii],2) ;
      ampl2ppbar[irap][ii]=pow(amplppbar[irap][ii],2);

      ampl2[irap][ii]+= repp*repp*(1-amplP)*(1-amplP);
      ampl2ppbar[irap][ii]+= reppbar*reppbar*(1-amplP)*(1-amplP);
 ...
 // uncomment for profile  std::cout << amplsd2[irap][ii]<<" ";
    }; // uncomment for profile  std::cout << std::endl;
  };
 ...
 ****************/
 ...
 std::cout << sigmasd2[irap][ibb-1]*10;// << " "<<sigmasd2[irap][ibb-1]*10;;
